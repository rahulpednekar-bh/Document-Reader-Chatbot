...
graph TB
    ...
    subgraph AzureServices["Azure Services"]
        Blob["Azure Blob Storage<br/>Container: documents<br/>(raw files)"]
    ...
    DF -- "store raw file" --> Blob
    DF -- "upload file + attach to Vector Store" --> Foundry
    DF -- "save document metadata" --> Cosmos
...
sequenceDiagram
    ...
    FN->>Blob: Upload file stream
    Blob-->>FN: blobUrl
    FN->>Foundry: Upload file (Files API)
    Foundry-->>FN: foundryFileId
    FN->>Foundry: Attach file to Vector Store
    FN->>Cosmos: Save DocumentMetadata {id, blobUrl, foundryFileId, status:"indexed"}
    FN-->>FE: {documentId, status: "indexed"}
    FE->>FE: Update progress bar to 100%
